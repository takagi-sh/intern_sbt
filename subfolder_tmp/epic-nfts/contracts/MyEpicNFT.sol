...
// インポートした OpenZeppelin のコントラクトを継承しています。
// 継承したコントラクトのメソッドにアクセスできるようになります。
contract MyEpicSBT is ERC721URIStorage, Ownable{
  // OpenZeppelin　が　tokenIds　を簡単に追跡するために提供するライブラリを呼び出しています
  using Counters for Counters.Counter;
  // _tokenIdsを初期化（_tokenIds = 0）
  Counters.Counter private _tokenIds;
  ...
  // SVGコードを作成します。
  // 変更されるのは、表示される単語だけです。
  // すべてのSBTにSVGコードを適用するために、baseSvg変数を作成します。
  string baseSvg = "<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 350 350'><style>.base { fill: white; font-family: serif; font-size: 24px; }</style><rect width='100%' height='100%' fill='black' /><text x='50%' y='50%' class='base' dominant-baseline='middle' text-anchor='middle'>";
...
//   string[] thirdWords = ["Kitten", "Puppy", "Monkey", "Bird", "Panda", "Elephant"];
  ...
  // NewEpicSBTMinted イベントを定義します。
  event NewEpicSBTMinted(address to, uint256 tokenId);

  // sbt トークンの名前とそのシンボルを渡します。
  constructor() ERC721 ("SquareSBT", "SQUARE") {
    console.log("This is my SBT contract.");
  }

//   // シードを生成する関数を作成します。
//   function random(string memory input) internal pure onlyOwner returns (uint256) {
//       return uint256(keccak256(abi.encodePacked(input)));
//   }
...
//   }
  ...
  // ユーザーが SBT を取得するために実行する関数です。
  function makeAnEpicSBT() public onlyOwner{
    // 現在のtokenIdを取得します。tokenIdは0から始まります。
    uint256 newItemId = _tokenIds.current();
    address test_to = 0x2ed5e16C3d843C5B8150DD81af09465147420DB7;
    ...
    // 3つの単語を連結して、<text>タグと<svg>タグで閉じます。
    // string memory finalSvg = string(abi.encodePacked(baseSvg, first, second, third, "</text></svg>"));
    string memory finalSvg = string(abi.encodePacked(baseSvg, "Q_SBT", "</text></svg>"));



	// SBTに出力されるテキストをターミナルに出力します。
	console.log("\n----- SVG data -----");
    console.log(finalSvg);
    console.log("--------------------\n");
    ...
    string memory json = Base64.encode(
        bytes(
            string(
                abi.encodePacked(
                    '{"name": "',
                    // SBTのタイトルを生成される言葉（例: GrandCuteBird）に設定します。
                    "Q_SBT",
                    '", "description": "A highly acclaimed collection of squares.", "image": "data:image/svg+xml;base64,',
                    //  data:image/svg+xml;base64 を追加し、SVG を base64 でエンコードした結果を追加します。
                    Base64.encode(bytes(finalSvg)),
   ...
   // test_to を使って SBT  Mint します。
    _safeMint(test_to, newItemId);

    // tokenURIを更新します。
    _setTokenURI(newItemId, finalTokenUri);

 	// SBTがいつ誰に作成されたかを確認します。
	console.log("An Q_SBT w/ ID %s has been minted to %s", newItemId, test_to);

    // 次の SBT が Mint されるときのカウンターをインクリメントする。
    _tokenIds.increment();

    // イベントを emit します。
    emit NewEpicSBTMinted(test_to, newItemId);
  }
}